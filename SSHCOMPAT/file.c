...
int _open(const char* filename, int flags/*, int mode*/)
{
	...
	asciiToUnicode(filename, wFileName);
...
{
	WCHAR* wMode = strdupAsciiToUnicode(mode);
...
{
	BOOL bSuccess;
	WCHAR* wPathName = strdupAsciiToUnicode(pathname);
...
int stat(const char* filename, struct stat* st)
{
	...
	wFileName = strdupAsciiToUnicode(filename);	
...
int truncate(const char *path, long length)
{
	int iRet = -1;
	HANDLE h;
	WCHAR* wFileName = strdupAsciiToUnicode(path);
...
int _mkdir(const char *pathname, int mode)
{
	...
	BOOL bRet;
	WCHAR* wzDirectoryName = strdupAsciiToUnicode(pathname);
