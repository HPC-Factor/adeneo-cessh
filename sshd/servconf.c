...
parse_filename:
		...
		if (!arg || *arg == '\0')
			fatal("%s line %d: missing file name.",
			    filename, linenum);
		if (*charptr == NULL) {
			//JJH *charptr = tilde_expand_filename(arg, getuid());
			*charptr = xstrdup(arg);
...
parse_flag:
	...
	case sStrictModes:
		...
		arg = strdelim(&cp);
		RETAILMSG(1,(TEXT("Strict mode not supported under CE\r\n")));
		//goto parse_flag;
		break;
	...
	case sUsePrivilegeSeparation:
		arg = strdelim(&cp);
		RETAILMSG(1,(TEXT("Privilege separation is not implemented !\r\n")));
		break;
...
#endif// WINCE_PORT
		arg = strdelim(&cp);
		RETAILMSG(1,(TEXT("Support for SSH2 only!\r\n")));
		intptr = &options->protocol;
		*intptr = SSH_PROTO_2;
		break;
